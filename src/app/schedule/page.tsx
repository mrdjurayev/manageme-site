...
export default async function SchedulePage() {
  ...
  const referenceTimeZone = Intl.DateTimeFormat().resolvedOptions().timeZone;
  ...
  return (
    <main className="min-h-screen bg-[#efefef] px-3 py-5 md:px-5 md:py-8">
      <div className="mx-auto flex min-h-[calc(100dvh-2.5rem)] max-w-[2000px] flex-col gap-0 md:min-h-[calc(100dvh-4rem)]">
        ...
        <LessonScheduleCanvas className="min-h-0 flex-1" referenceDate={referenceDate} referenceTimeZone={referenceTimeZone} />
      </div>
    </main>
  );
}
